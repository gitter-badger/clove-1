...
class BlockcypherAPI(object):
    ...
    def get_transaction(cls, tx_address: str) -> dict:
        return clove_req_json(f'{cls.blockcypher_url()}/txs/{tx_address}')
    ...
    def get_balance(cls, wallet_address: str) -> float:
        data = clove_req_json(f'{cls.blockcypher_url()}/addrs/{wallet_address}/full?limit=2000')
        if data is None:
